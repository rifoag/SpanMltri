...
class SpanMltri(nn.Module):
    def __init__(self, encoder_model_name_or_path="indolem/indobert-base-uncased",
                 d_hidden=768, max_sentence_length=40, num_of_te_class=3, num_of_relation_class=4, max_span_length=8, k=0.4):
        ...
        self.relation_scorer = RelationScorer(d_hidden, max_sentence_length, num_of_relation_class, k)
        
    def forward(self, x):
        x = self.base_encoder(x)
        x, span_ranges = self.span_generator(x)
